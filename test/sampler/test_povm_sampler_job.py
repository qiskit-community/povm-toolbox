...
class TestPOVMSamplerJob(TestCase):
    ...
    def test_recover_job(self):
        ...
        job.save_metadata(filename="saved_metadata")
        tmp = job.base_job

        def job_loader(job_id: str) -> BasePrimitiveJob:
            if job_id != tmp.job_id():
                raise ValueError
            return tmp

        job_recovered = POVMSamplerJob.recover_job(
            metadata_filename="saved_metadata", job_loader=job_loader
        )
