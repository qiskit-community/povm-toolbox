...
class TestDualFromState:
    ...
    def test_implemented(self):
        ...
        assert dual.is_dual_to(joint_povm)

    def test_optimal_dual(self, subtests):
        """Test that the method constructs a valid dual."""
        joint_povm: MultiQubitPOVM | SingleQubitPOVM = self.povm
        state = random_density_matrix(2, seed=self.SEED)
        ...
        with subtests.test("Test canonical dual."):
            ...
            assert canonical_dual.is_dual_to(joint_povm)
            assert np.isclose(exp_val_canonical, exact_exp_val)
            assert np.isclose(
                var_canonical,
                np.dot(probabilities, np.power(canonical_weights, 2)) - exact_exp_val**2,
            )

        with subtests.test("Test optimal dual."):
            ...
            assert optimal_dual.is_dual_to(joint_povm)
            assert np.isclose(exp_val_optimal, exact_exp_val)
            assert np.isclose(
