...
class TestProductPVMSimPOVMImplementation(TestCase):
    ...
    def test_CS_build(self):
        ...
        q = [1./3., 1./3., 1./3.]
        sqpovm = SingleQubitPOVM(np.array([q[0]* self.Z0,q[0]* self.Z1,q[1]* self.X0,q[1]* self.X1,q[2]* self.Y0,q[2]* self.Y1]))
    ...
    def test_LBCS_build(self):
        ...
        for n_qubit in range(1,11):
            ...
            cs_povm = cs_implementation.to_povm()
            for i in range(n_qubit):
                sqpovm = SingleQubitPOVM(np.array([q[i,0]* self.Z0,q[i,0]* self.Z1,q[i,1]* self.X0,q[i,1]* self.X1,q[i,2]* self.Y0,q[i,2]* self.Y1]))
                self.assertEqual(cs_povm.povm_list[i].n_outcomes, sqpovm.n_outcomes)
                for k in range(sqpovm.n_outcomes):
